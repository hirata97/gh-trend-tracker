...
beforeAll(async () => {
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS languages (
  ...
  // テストデータを挿入
  await db.prepare(`INSERT INTO repositories (repo_id, name, full_name, owner, language, description, html_url, created_at, updated_at)
    VALUES (1, 'react', 'facebook/react', 'facebook', 'JavaScript', 'A declarative UI library', 'https://github.com/facebook/react', '2024-01-01', '2024-06-01')`).run();
  await db.prepare(`INSERT INTO repositories (repo_id, name, full_name, owner, language, description, html_url, created_at, updated_at)
    VALUES (2, 'vue', 'vuejs/vue', 'vuejs', 'TypeScript', 'Progressive JavaScript Framework', 'https://github.com/vuejs/vue', '2024-01-01', '2024-06-01')`).run();
  await db.prepare(`INSERT INTO repositories (repo_id, name, full_name, owner, language, description, html_url, created_at, updated_at)
    VALUES (3, 'svelte', 'sveltejs/svelte', 'sveltejs', 'TypeScript', 'Cybernetically enhanced web apps', 'https://github.com/sveltejs/svelte', '2024-01-01', '2024-06-01')`).run();

  await db.prepare(`INSERT INTO repo_snapshots (repo_id, stars, forks, watchers, open_issues, snapshot_date) VALUES (1, 230000, 47000, 6700, 900, '2026-02-09')`).run();
  await db.prepare(`INSERT INTO repo_snapshots (repo_id, stars, forks, watchers, open_issues, snapshot_date) VALUES (2, 210000, 34000, 6300, 600, '2026-02-09')`).run();
  await db.prepare(`INSERT INTO repo_snapshots (repo_id, stars, forks, watchers, open_issues, snapshot_date) VALUES (3, 80000, 4200, 1200, 800, '2026-02-09')`).run();

  await db.prepare(`INSERT INTO metrics_daily (repo_id, calculated_date, stars_7d_increase, stars_30d_increase, stars_7d_rate, stars_30d_rate) VALUES (1, '2026-02-09', 500, 2000, 0.0022, 0.0088)`).run();
  await db.prepare(`INSERT INTO metrics_daily (repo_id, calculated_date, stars_7d_increase, stars_30d_increase, stars_7d_rate, stars_30d_rate) VALUES (2, '2026-02-09', 300, 1200, 0.0014, 0.0057)`).run();
  ...
  describe('successful responses', () => {
    ...
    it('accepts all valid sort_by values', async () => {
      ...
      for (const sortBy of sortValues) {
        const response = await SELF.fetch(
          `http://example.com/api/trends/daily?sort_by=${sortBy}`
        );
        expect(response.status).toBe(200);
      }
    });
