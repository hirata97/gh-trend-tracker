...
| [障害対応Runbook](./docs/プロセス/障害対応Runbook.md)                | 障害種別ごとの初動手順   | 障害発生時の対応   |

## 🎯 開発フェーズ

| Phase                | 状態   | 主な機能                                                                                                            |
| -------------------- | ------ | ------------------------------------------------------------------------------------------------------------------- |
| **Phase 1 (MVP)**    | 完了   | トレンドランキング一覧（Top 100）、言語フィルタリング、7日/30日スター増加数表示、リポジトリ詳細（90日間推移グラフ） |
| **Phase 2 (拡張)**   | 完了   | リポジトリ名検索、お気に入り（ローカルストレージ）、週別トレンドランキング、ソート機能                              |
| **Phase 3 (収益化)** | 計画中 | GitHub OAuth認証、AI要約機能（課金/無料プレビュー）、Stripe決済連携                                                 |
...
- ✅ **CORS**: 本番環境（`ENVIRONMENT=production`）では`ALLOWED_ORIGINS`未設定時に全拒否（fail-closed）対応済み
