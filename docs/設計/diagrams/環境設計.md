...
| `SENTRY_DSN`（Backend） | `.dev.vars` | `wrangler secret put` | 不要（DSN変更時） |
| `PUBLIC_SENTRY_DSN`（Frontend） | `.env` | Cloudflare Pages 環境変数 | 不要（DSN変更時） |
| `SENTRY_AUTH_TOKEN`（Frontend build） | `.env` | Cloudflare Pages 環境変数 | 不要（token変更時） |
