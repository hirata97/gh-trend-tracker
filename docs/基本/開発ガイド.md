...
Frontend は Cloudflare Pages（Direct Upload モード）でホストする。
Git統合は使わず、CI から `wrangler pages deploy` で明示的にデプロイする。

#### 環境別デプロイフロー

| 環境 | トリガー | Backend向き先 | ワークフロー |
| ---- | -------- | ------------- | ------------ |
| **ローカル** | 手動 | `localhost:8787` | - |
| **Preview** | PRオープン/更新 | 開発Workers（`DEV_API_URL`） | `deploy-preview.yml` |
| **本番** | `main` へのpush | 本番Workers（`PROD_API_URL`） | `deploy.yml` |

#### 必要なGitHub Secrets

| シークレット名 | 説明 |
| -------------- | ---- |
| `CLOUDFLARE_API_TOKEN` | Pages:Edit権限を持つCloudflare APIトークン |
| `CLOUDFLARE_ACCOUNT_ID` | CloudflareアカウントID |
| `PROD_API_URL` | 本番BackendのURL（例: `https://gh-trend-tracker-api.xxx.workers.dev`） |
| `DEV_API_URL` | 開発BackendのURL（Preview環境向け） |

#### Cloudflare Pages プロジェクト初回セットアップ（手動・初回のみ）

1. Cloudflare ダッシュボード → Pages → Create a project
2. **Direct Upload** モードを選択
3. プロジェクト名: `gh-trend-tracker`
4. 初回ダミーデプロイ（ローカルから）:
   ```bash
   cd apps/frontend
   npm run build
   CLOUDFLARE_API_TOKEN=<your-token> npx wrangler pages deploy dist --project-name=gh-trend-tracker
   ```
5. GitHub Secrets に `CLOUDFLARE_API_TOKEN`・`CLOUDFLARE_ACCOUNT_ID`・`PROD_API_URL`・`DEV_API_URL` を登録
6. GitHub リポジトリ変数 `CLOUDFLARE_PAGES_ENABLED=true` を設定してPreviewデプロイを有効化
   - Settings > Secrets and variables > Variables > New repository variable

#### PR Preview デプロイの確認方法

PRを作成すると `deploy-preview.yml` が自動実行され、以下のURLでPreviewが閲覧できる:

```
https://<branch-name>.gh-trend-tracker.pages.dev
```
...
毎月1日 UTC 03:00 に [月次リストアテスト workflow](.github/workflows/restore-test.yml) が自動実行されます。このワークフローは以下の手順で **`gh-trends-db-dev`（開発用DB）を一時的に初期化**します：

1. 開発DBの全テーブルをドロップ
2. R2 から最新バックアップを取得して適用
3. 整合性チェックを実行

> ⚠️ **リストアテスト実行中は開発DBが一時的に破壊状態になります。**
> 実行時間帯（UTC 03:00前後）に開発DBを使った作業は避けてください。
> 手動実行（`workflow_dispatch`）を行う場合も同様に、実行前にチームへ周知してください。
